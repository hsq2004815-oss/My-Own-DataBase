"""Build a local SQLite + FTS index for UI reference records."""
...
def insert_reference(conn: sqlite3.Connection, record: dict[str, Any]) -> None:
    ...
    conn.execute(
        ...
        (
            ...
            record["source"]["license_notes"],
        ),
    )
    conn.execute(
        """
        INSERT OR REPLACE INTO references_meta
        (record_id, source_id, source_name, source_url, page_type, signal_strength,
         evidence_level, ai_summary, record_json)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        """,
        (
            record["record_id"],
            ...
            record["source"]["url"],
            record["page_type"],
            record["quality"]["signal_strength"],
            record["quality"]["evidence_level"],
            record["retrieval"]["ai_summary"],
...
def insert_chunk(conn: sqlite3.Connection, chunk: dict[str, Any], has_fts: bool) -> None:
    conn.execute(
        ...
        (chunk_id, record_id, source_name, source_url, page_type, section, content, tokens, metadata_json)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        """,
        (
            chunk["chunk_id"],
    ...
    if has_fts:
        conn.execute(
            """
            INSERT INTO chunks_fts (chunk_id, record_id, page_type, section, content)
            VALUES (?, ?, ?, ?, ?)
            """,
            (
                chunk["chunk_id"],
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build SQLite index for UI references.")
    ...
    chunk_files = [path for path in iter_json(args.chunks) if path.name != "all_reference_chunks.json"]
    ...
    fts_status = "enabled" if has_fts else "unavailable"
    print(
        f"indexed {len(reference_files)} reference record(s) and {chunk_count} chunk(s) "
        f"to {args.db} (fts5={fts_status})"
    )
    return 0
