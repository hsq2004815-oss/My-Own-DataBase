...
```powershell
python E:\DataBase\scripts\ui_design\search_ui.py "premium UI execution visual anchor non generic layout" --limit 8
python E:\DataBase\scripts\ui_design\search_ui.py "高级 UI 视觉锚点 技术可视化 非模板" --limit 8
python E:\DataBase\scripts\ui_design\search_ui.py "light liquid glass workspace project specific visuals" --limit 8
python E:\DataBase\scripts\ui_design\search_ui.py "设计自检 glass quality component density" --limit 8
...
- `search_ui.py "premium UI execution visual anchor non generic layout" --limit 8`: returns `implementation-premium-ui-execution-quality-rules` first.
- `search_ui.py "高级 UI 视觉锚点 技术可视化 非模板" --limit 8`: returns `implementation-premium-ui-execution-quality-rules` first.
- `search_ui.py "light liquid glass workspace project specific visuals" --limit 8`: returns execution quality plus Liquid Glass workspace rules.
- `search_ui.py "设计自检 glass quality component density" --limit 8`: returns `implementation-premium-ui-execution-quality-rules` first.
...
- Premium UI Execution Quality: 执行质量层，不替代具体 hero、Liquid Glass、motion、typography 或 workspace pattern；用于要求 agent 选择视觉方向、视觉锚点、组件系统、CSS token、素材使用策略和设计自检，防止高级规则召回后仍输出普通模板。
