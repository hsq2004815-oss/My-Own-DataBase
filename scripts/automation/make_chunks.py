"""Create retrieval chunks from automation reference records."""
...
def make_chunk(record: dict[str, Any], section: str, title: str, body: str) -> dict[str, Any]:
    content = (
        ...
        f"Automation type: {record['automation_type']}\n"
        f"Tooling: {', '.join(record.get('tooling', []))}\n"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Create automation chunks.")
    ...
    write_json(args.output_dir / "all_automation_chunks.json", all_chunks)
    print(f"created {len(all_chunks)} chunk(s) from {len(files)} automation record(s)")
    return 0
