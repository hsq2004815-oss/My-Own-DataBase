"""Normalize and validate agent workflow reference records."""
...
TEMPLATE_PATH = ROOT / "common" / "templates" / "agent_workflow_reference.template.json"
TAXONOMY_PATH = ROOT / "common" / "taxonomy" / "agent_workflow_types.json"
DEFAULT_INPUT_DIR = ROOT / "domains" / "agent_workflow" / "processed" / "references"
...
def workflow_types() -> set[str]:
    data = load_json(TAXONOMY_PATH)
    return {item["id"] for item in data.get("workflow_types", [])}
...
def validate_record(record: dict[str, Any], allowed_types: set[str], path: Path) -> None:
    required = ["record_id", "source", "domain", "workflow_type", "agent_targets", "quality", "retrieval"]
    missing = [key for key in required if not record.get(key)]
    if missing:
        raise ValidationError(f"{path}: missing required fields: {', '.join(missing)}")
    if record["workflow_type"] not in allowed_types:
        raise ValidationError(f"{path}: unknown workflow_type '{record['workflow_type']}'")
    ...
    if quality.get("signal_strength") not in {"high", "medium", "low"}:
        raise ValidationError(f"{path}: invalid signal_strength")
    if quality.get("evidence_level") not in {"tested", "documented", "draft", "observed"}:
        raise ValidationError(f"{path}: invalid evidence_level")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Normalize agent workflow references.")
    parser.add_argument("--input", type=Path, default=DEFAULT_INPUT_DIR)
    parser.add_argument("--output-dir", type=Path, default=DEFAULT_OUTPUT_DIR)
    parser.add_argument("--check", action="store_true")
    args = parser.parse_args()
    template = load_json(TEMPLATE_PATH)
    allowed_types = workflow_types()
    ...
    print(("validated" if args.check else "normalized") + f" {len(records)} agent workflow record(s)")
    return 0
