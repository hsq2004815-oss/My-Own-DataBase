...
- Backend Engineering: 338 chunks / 28 references / 12 topics / 12 patterns / 10 checklists / 12 templates
- 当前可用 SQLite 索引位于 `runtime/db/sqlite/*`。Backend Engineering 已写入 `runtime/db/sqlite/backend/backend_references.db`，并已接入 `/backend/search` 与 `/brief`。
...
│  ├─ dev_tools/             # GitHub 下载加速、开发辅助工具、Agent 工具链规则
│  └─ voice_assistant/       # 小黄语音唤醒、STT/TTS 引擎调研、桌面语音助手参考
├─ runtime/
...
│  └─ backend/               # Backend Engineering processed/SQLite 管线
└─ TASK_MEMORY.md            # 维护记录和下一步注意事项
```

## 可复用代码资产层 (Code Assets)

数据库新增了"可复用代码资产层"，用于把已有开源项目/代码资料整理成通用代码能力，服务小黄、Windows 工具、自动化工具、后端项目、小程序等项目开发。

代码资产位于：

- `common/schemas/code_asset.schema.json` — 代码资产记录 schema
- `common/taxonomy/code_asset_categories.json` — 能力分类 taxonomy
- `domains/<domain>/processed/code_assets/` — 结构化代码资产记录 (JSON)
- `domains/<domain>/wiki/snippets/` — 人类可读的可复用片段说明
- `domains/<domain>/wiki/adapters/` — 适配到用户项目的方案
- `domains/<domain>/wiki/indexes/` — 能力索引，供 Agent 快速查找
- `domains/agent_workflow/rules/code-asset-reuse-rules.md` — 代码资产复用规则

代码资产是 `processed/wiki` 层资产，不替代现有 `raw/processed/wiki/rules/references`。`raw` 仍然只是原始资料入口，Agent 使用时先查 `indexes/code_assets`，再查 `snippets/adapters`，不默认扫 raw。复用代码前必须检查 license、依赖、适配层和测试。
...
Backend Engineering 已完成 `rules/`、`wiki/`、`references/`、processed chunks、SQLite/FTS5 索引和 API/brief 接入。curated docs 仍是规则源，`runtime/db/sqlite/backend/backend_references.db` 是本机智能体检索入口。
...
后端/API/数据库/登录权限/部署/RAG 任务优先使用本机 API 或 brief CLI 的 backend 接入：

```powershell
python E:\DataBase\scripts\brief.py "设计 FastAPI PostgreSQL JWT RBAC 后端，并支持 RAG SSE 流式输出" --ui 1 --workflow 2 --automation 0 --backend 8 --assets 0
```

也可以直接检索 backend chunks：

```powershell
Invoke-RestMethod "http://127.0.0.1:8765/backend/search?q=JWT%20RBAC%20auth%20permission&limit=5"
```

如果 API 未启动，再按 curated docs 入口读取：
...
开发工具领域位于 `domains/dev_tools`。当前用于整理 GitHub 下载加速工具和 Agent 下载流程：

- GitHub 仓库工具可以只保留 `processed/metadata`、`wiki`、`rules` 等 curated 产物进入版本库。
- 大型 raw 源码可以保留在本地 `domains/dev_tools/raw`，作为维护阶段只读资料，不要求默认上传。
- Agent 使用 dev_tools 时应优先读取 `domains/dev_tools/rules` 和 `domains/dev_tools/wiki`，不要默认整库读取 raw 第三方源码。
- GitHub 下载策略优先 `git clone --depth=1`，失败后再按 rules/wiki 中的 fgit、fastgit、gh-proxy、github-proxy fallback 顺序处理。
...
- `GET /backend/search`: 检索 Backend Engineering chunks。
...
注意：Backend Engineering 已接入 API 检索。普通调用不要重建 backend 索引或直接读取 raw GitHub 项目源码；维护阶段才运行 `scripts/backend/*`。
...
7. Backend Engineering 运行时索引来自 `domains/backend/processed/retrieval/backend-retrieval-chunks.jsonl`；普通任务只调用 `/brief` 或 `/backend/search`，不要重建索引。
8. 更新检索逻辑后至少运行：
