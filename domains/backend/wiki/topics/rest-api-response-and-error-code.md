# REST API Response And Error Code

## 主题说明
统一响应格式、错误码、分页和 request_id。

## 为什么重要
统一错误和响应让 Agent 能稳定生成前后端交互代码。
...
## 小项目推荐方案
使用 `{code,message,data,request_id}` 或等价结构。

## 中型项目推荐方案
错误码注册表、审计日志、跨服务 trace_id。

## 大型项目才需要的方案
多语言错误中心和复杂错误编排只在大型平台需要。
