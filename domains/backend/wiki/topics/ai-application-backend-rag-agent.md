# AI Application Backend RAG Agent

## 主题说明
AI 应用后端、RAG、Agent API、SSE 和向量存储。

## 为什么重要
AI 后端不仅是调用模型，还要处理用户隔离、文件 ingestion、检索、安全和超时。
...
## 小项目推荐方案
FastAPI + PostgreSQL/pgvector + SSE + 文件上传隔离。

## 中型项目推荐方案
任务队列、对象存储、多模型、OpenAI-compatible API、成本监控。

## 大型项目才需要的方案
多 Agent 平台、模型网关、多租户隔离只在产品明确需要时使用。
