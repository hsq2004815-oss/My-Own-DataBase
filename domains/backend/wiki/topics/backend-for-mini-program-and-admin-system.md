# Backend For Mini Program And Admin System

## 主题说明
小程序后端和管理系统后端的常见边界。

## 为什么重要
这类项目最容易遗漏权限、分页、错误码、审计和文件上传安全。
...
## 小项目推荐方案
模块化单体 + 用户/角色 + REST API + PostgreSQL/SQLite。

## 中型项目推荐方案
RBAC、菜单权限、操作审计、文件存储、队列、部署监控。

## 大型项目才需要的方案
微服务和复杂多租户隔离只在组织/租户模型明确时使用。
