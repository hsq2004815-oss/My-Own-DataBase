...
## 为什么重要
从模块化单体开始，按业务域分层，把 API、数据库、鉴权、日志和部署边界先建清楚。
...
## 小项目推荐方案
FastAPI/Express 单体 + SQLite 或 PostgreSQL；Redis/队列可选。

## 中型项目推荐方案
升级 PostgreSQL、Redis、后台任务、RBAC、CI、OpenTelemetry。

## 大型项目才需要的方案
微服务、K8s、服务网格、复杂 DDD、CQRS、Event Sourcing 只在团队、规模和运维能力明确时使用。
