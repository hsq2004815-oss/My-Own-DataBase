...
## 主题说明
认证、授权、JWT、session、OAuth2/OIDC 和 RBAC。

## 为什么重要
认证回答你是谁，授权回答你能做什么，二者不能混用。
...
## 小项目推荐方案
账号密码 + JWT 或 session；管理员后台使用 RBAC。

## 中型项目推荐方案
refresh token rotation、OAuth2/OIDC、细粒度 permission、审计日志。

## 大型项目才需要的方案
复杂 IAM、多租户高级隔离只在企业级需求明确时使用。
