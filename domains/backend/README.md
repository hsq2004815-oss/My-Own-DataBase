...
如果本机 `backend_api` 已启动，普通 backend 任务优先调用：

- `POST http://127.0.0.1:8765/brief`，设置 `backend_limit` 大于 0。
- `GET http://127.0.0.1:8765/backend/search?q=<query>&limit=<n>`，做 backend-only 精确检索。

如果 API 未启动或需要人工审查规则，再读取 curated docs：
...
- `processed/retrieval/backend-retrieval-chunks.jsonl`
- `processed/manifest/backend-index-manifest.json`
...
当前这条链路已经完成到 API 检索接口。后续扩展时仍按上述顺序追加，不要直接改 runtime DB 作为唯一来源。
...
- `processed/retrieval/backend-retrieval-chunks.jsonl` 已有 338 个可检索 chunks。
- `processed/manifest/backend-index-manifest.json` 记录 SQLite/FTS5 index 构建状态。
- `runtime/db/sqlite/backend/backend_references.db` 已有 `backend_chunks` 和 `backend_chunks_fts`。
- `backend_api` 已提供 `/backend/search`，`/brief` 会在后端任务中返回 `backend_queries` 和 `backend_chunks`。
- `output/reports/backend-phase-2b-wiki-references-report.md`、`output/reports/backend-phase-2b-audit-and-repair-report.md`、`output/reports/backend-phase-2c-retrieval-prep-report.md` 和 `output/reports/backend-phase-2d-sqlite-bm25-index-report.md` 记录阶段结果。

## 下一阶段

下一阶段建议做质量改进，而不是重复建库：

1. 根据真实 agent 调用日志优化 backend query rules 和 ranking。
2. 给高频任务补充更细粒度 references 或 templates。
3. 增加 backend-only 回归检索用例。
4. 需要新增资料时，先更新 curated docs 和 processed chunks，再重建 SQLite。
